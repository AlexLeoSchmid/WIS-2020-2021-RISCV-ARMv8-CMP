...
\label{ref:conclusion}
	...
	While ARM features a large and complex base \gls{ISA} with only limited support for custom extensions, RISC-V features a much more modular
	approach with a very small base \gls{ISA} and most features being specified in the form of optional extensions with custom extensions being
	widespread and encouraged.

	While there are minor differences in code size between the two \glspl{ISA} with ARM having a smaller 32-bit code size and RISC-V having a
	...
	Lastly, the ecosystem is a major advantage of ARM with extensive toolchain and compiler support in addition to most major operating systems
	supporting ARM in some form or another, as described in chapter \ref{ecosystem}.
	...
	by only implementing the base \gls{ISA} and only implementing additional extensions as needed.
	This modularity is especially useful in embedded applications, where small \glspl{ISA} are preferred due to the need
	for high power efficiency and a comparatively smaller need to high processing power \cite{Blem2013}.

	This modularity does pose the risk of excessive fragmentation of the \gls{CPU} market, with different \glspl{CPU} supporting
	different extensions, not allowing software to be reused across these \glspl{CPU}. This fragmentation can however be avoided
	by \gls{CPU} designers and operating system developers agreeing upon a set of extensions necessary for certain use cases.

	The fact that there are only minor differences in performance between the two \glspl{ISA} shows that RISC-V can, in principle,
	compete with established \glspl{ISA} in all applications, from embedded to high performance computing.
	...
	Meanwhile, improved compiler and operating system support for RISC-V remain important challenges for software developers and
