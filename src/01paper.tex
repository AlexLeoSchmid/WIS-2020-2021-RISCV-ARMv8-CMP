...
are an important factor when deciding which \gls{ISA} to use. The two \gls{ISA}'s business models are compared in chapter \ref{businessModels}.

The \gls{ISA}'s \textbf{complexity} refers to the amount of effort required to implement the \gls{ISA}. The more complex an \gls{ISA} is, the more developer
...
increasing a \gls{CPU}'s development cost \cite{Patterson1980}. For the comparison of both \glspl{ISA}'s complexity, see chapter \ref{complexity}.

The next aspect is a \gls{CPU}'s \textbf{performance}. There are various aspects that make up a \gls{CPU}'s performance.
Among these are execution speed, code size and power efficiency.
...
\glspl{ISA} often allow for a number of instruction set \textbf{extensions} that may or may not be implemented by a given \gls{CPU}.
The choice of extensions greatly influence the flexibility of an \gls{ISA} and its performance for specific use cases.
Thus they are an important factor to consider which \gls{ISA} to implement for a new \gls{CPU}.
...
that can run on that \gls{CPU}. This is especially important in consumer desktop and mobile devices where a large variety of software is to be executed.
	...
	%TODO: Format, add sources, formulate new

	Getting access to the ISA is only the first step in producing and offering a self designed silicon.
	Before this there are several rough main steps to go through:
	\begin{enumerate}
		\item Designing the CPU Core.
		\item Checking for comliance to the ISA.
		\item Testing the core for formal correctness, function with random data and function with edge case data.
		\item Finding a factory partner for production.
		\item Creating a at least a software toolchain for creating and comiling applications on your hardware. The toolchain must also include CMSIS package and should undergo sufficent testing.
	\end{enumerate}
	% ARM "Steps designing your own chip"
	ARM gives you a considerable headstart in this todo list: They already supply a completet CPU Core, which is compliant to their ISAs and tested formally aswell as in function. It is also proven many times in already existing hardware. The ARM package also includes a complete software toolchain which enables chip designers to create their own specced chips whithin the ARM environment. Because you are not able to leave the arm environment, they are also able to offer a complete compiler toolchain and CMSIS packages for all their silicon %anders formulieren
	extensions. They offer connections to chip foundries who already have experience in manufacturing ARM based silicon.

	When starting with RISC-V, you have to start from the ground up. RISC-V International only supplies the ISA Specification. From there on you have to find your own way. If you want, you could chose to work down the above list yourself, but there is also the alternative to choose from an already designed, probably also compliance checken and tested cpu core. But keep in mind that all the implementations come with different licenses and an different ecosystem aswell. %TODO: Rest der kette...
